...
If the model satisfies $p(y \mid \theta) = f(y - \theta)$, shifting the origin by a constant $c$ leaves the physics unchanged: $y' = y + c \implies \theta' = \theta + c$.
...
If the model satisfies $p(y \mid \sigma) = \frac{1}{\sigma} f(y / \sigma)$, scaling the coordinate units by $\lambda > 0$ yields $y' = \lambda y \implies \sigma' = \lambda \sigma$.
...
$$\frac{\partial \ln p(y \mid \phi)}{\partial \phi_i} = \sum_k \frac{\partial \ln p(y \mid \theta)}{\partial \theta_k} \frac{\partial \theta_k}{\partial \phi_i} = \sum_k J_{ki} S_k(\theta)$$
...
$$\sqrt{\det \tilde{F}(\phi)} = \lvert \det J\rvert \sqrt{\det F(\theta)}$$

Applying the transformation law of probability densities:
$$p_\Phi(\phi) = p_\Theta(\theta) \lvert \det J\rvert = \sqrt{\det F(\theta)} \lvert \det J\rvert = \sqrt{\det \tilde{F}(\phi)}$$
