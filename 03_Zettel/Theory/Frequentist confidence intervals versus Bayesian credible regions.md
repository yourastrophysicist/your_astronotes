...
$$P\left( L(D) \le \theta_{\rm true} \le U(D) \,\middle|\, \theta_{\rm true} \right) = 1 - \alpha$$
...
A Bayesian conditions directly on the actual observed dataset $D_0$ and treats degree of belief regarding parameter $\theta$ as a probability distribution $p(\theta|D_0, I)$.

A $1-\alpha$ credible interval $[a, b]$ satisfies:
$$P\left( a \le \theta \le b \,\middle|\, D_0, I \right) = \int_a^b p(\theta|D_0, I) \, d\theta = 1 - \alpha$$
...
2. **Highest Posterior Density (HPD) region**: The set of points where the posterior density exceeds a threshold, $\{\theta : p(\theta|D_0) \ge \gamma\}$, ensuring the shortest possible interval for unimodal posteriors and naturally handling multimodality.
...
$$p(n|k, \theta) = \binom{n-1}{k-1} \theta^k (1-\theta)^{n-k}$$

Both experiments yield identical likelihood functions up to an irrelevant constant:
$$L(\theta) \propto \theta^3 (1-\theta)^9$$

- **Bayesian result**: The posterior $p(\theta|D) \propto L(\theta)p(\theta)$ is identical regardless of the experimenter's stopping intentions.
