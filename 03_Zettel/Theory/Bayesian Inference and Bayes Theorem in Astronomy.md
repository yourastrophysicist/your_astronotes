...
## Bayes Theorem
$$p(\boldsymbol{\theta} | \boldsymbol{D}, M) = \frac{p(\boldsymbol{D} | \boldsymbol{\theta}, M) \, p(\boldsymbol{\theta} | M)}{p(\boldsymbol{D} | M)}$$
where:
- $\boldsymbol{\theta}$: physical parameters of interest.
- $\boldsymbol{D}$: observed data array.
- $M$: assumed physical model.
- $p(\boldsymbol{\theta} | \boldsymbol{D}, M)$: **posterior probability density function** (updated state of knowledge).
- $p(\boldsymbol{D} | \boldsymbol{\theta}, M) = \mathcal{L}(\boldsymbol{\theta})$: **likelihood function** (probability of data given parameters).
- $p(\boldsymbol{\theta} | M) = \pi(\boldsymbol{\theta})$: **prior probability density** (knowledge prior to observation).
- $p(\boldsymbol{D} | M) = \mathcal{Z}$: **Bayesian evidence** (marginal likelihood), defined as:
...
$$p(\boldsymbol{\theta} | \boldsymbol{D}) \propto \mathcal{L}(\boldsymbol{\theta}) \, \pi(\boldsymbol{\theta})$$
