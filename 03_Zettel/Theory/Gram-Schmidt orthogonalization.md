...
with $Q$ orthogonal ($Q^T Q = I$) and $R$ upper triangular with diagonal $\\lvert \mathbf{u}_i\\rvert$ and off-diagonal $(\mathbf{q}_k \cdot \mathbf{a}_i)$. this is [[QR decomposition]] — Gram-Schmidt is *one* way to compute it.
