...
a galaxy's spectrum is not flat, so $N(m)$ depends *strongly* on the band you use. each wavelength is a different selection function on the galaxy population, and each has its own [[K-correction]], so the cosmological information you extract is different.
...
the Durham website gives postscript plots and tabulated text data for each band; pablo recommends pulling the K-band table for the LF normalization in the [[Schechter K-band luminosity function]] note.
...
see [[K-correction in optical vs sub-mm]] for the explicit form.
...
- previous: [[Galaxy number counts N(m)]], [[Differential vs cumulative number counts]]
- band conversions: [[K-correction in optical vs sub-mm]]
- morphology split (in B-band): [[Driver 1998 counts by morphology]]
