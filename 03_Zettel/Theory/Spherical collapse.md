...
the spherical collapse threshold is the input to the **Press-Schechter halo mass function** (see [[Press-Schechter halo mass function]]). the idea: given the linear matter power spectrum $P_m(k)$, smooth it on scale $R$ to get the variance $\sigma^2(R)$ of the linear $\delta$ field. the fraction of mass in collapsed halos of mass $> M(R)$ is approximately the probability that a Gaussian field exceeds $\delta_c$:
