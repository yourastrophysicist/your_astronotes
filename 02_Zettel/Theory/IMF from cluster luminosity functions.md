...
2. apply a theoretical or empirical **mass-luminosity relation** $M = M(L)$ from Stellar evolution tracks for the cluster's age and [metallicity](../../02_Zettel/Theory/Stellar populations I II III.md),
3. use the chain rule:

$$\xi(M) = \frac{dN}{dM} = \Phi(M_V) \, \left| \frac{dM_V}{dM} \right|.$$

the jacobian $|dM_V/dM|$ is the local slope of the [M-L relation](../../02_Zettel/Theory/Mass-luminosity relation.md) on the MS.
...
- [JWST](../../02_Zettel/Theory/JWST and the first stars.md) NIRCam: extends to $\sim 0.05\,M_\odot$ and into the substellar/brown-dwarf regime in nearby clusters and the Magellanic clouds.

**field-vs-cluster comparison**

in young open clusters (e.g. orion, NGC 6611), the LF is close to the [IMF](../../02_Zettel/Theory/Initial vs present-day mass function.md) because relaxation has not yet altered the population. in old [GCs](../../02_Zettel/Theory/Globular Clusters.md), the observed LF is strongly modified by mass segregation and evaporation, so the inferred IMF requires forward modelling.

a useful complement is to fit the cluster LF jointly with [CMD](../../02_Zettel/Theory/Color-magnitude diagrams of clusters.md) features (turnoff, [RGB](../../02_Zettel/Theory/Red giant branch RGB.md) tip, [SGB](../../02_Zettel/Theory/Subgiant branch SGB.md) luminosity) which constrain age and metallicity simultaneously, fixing $M_V(M)$ self-consistently.

**uncertainties**

- M-L relation uncertainties (especially at low mass; differences between models reach $\sim 0.05\,M_\odot$),
- [unresolved binaries](../../02_Zettel/Theory/Cluster binary fraction methods.md) inflate the LF at $\sim 0.75$ mag above the single-star ridge and bias the inferred $\xi(M)$,
...
modern best results give cluster IMFs consistent with [chabrier](../../02_Zettel/Theory/Salpeter Kroupa Chabrier IMFs.md) above $0.1\,M_\odot$, with hints of cluster-to-cluster variations at the $\sim 10\text{--}20\%$ level on the low-mass slope.
