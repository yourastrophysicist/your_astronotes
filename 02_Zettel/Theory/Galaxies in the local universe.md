...
a galaxy is a **gravitationally bound system** of $10^7$–$10^{12}$ stars, gas, dust, and dark matter. they are the **fundamental building blocks** of cosmic structure on intermediate scales (kpc-Mpc).

---

## the Hubble flow and the local universe

at low redshift ($z \lesssim 0.1$, distance $\lesssim$ 400 Mpc), the universe looks **clumpy on small scales** but has the **Hubble flow** $v = H_0 d$ on average. peculiar motions ($\sim 100-1000$ km/s) deviate from the Hubble flow at the few-percent level for nearby galaxies.
...
our home group: $\sim$ 50 galaxies within 1 Mpc of the Milky Way.

major members:
| galaxy | type | mass ($M_\odot$) | distance |
|---|---|---|---|
| Milky Way | SBbc | $10^{12}$ (with halo) | 0 |
| Andromeda (M31) | SAb | $10^{12}$ | 750 kpc |
| Triangulum (M33) | Scd | $5 \times 10^{10}$ | 850 kpc |
| Large Magellanic Cloud (LMC) | irregular | $10^{10}$ | 50 kpc |
| Small Magellanic Cloud (SMC) | irregular | $10^9$ | 60 kpc |

plus dozens of dwarf spheroidals, dwarf irregulars, ultra-faint dwarfs (some discovered only recently).

the Milky Way and Andromeda are on a **collision course** — they will merge in $\sim 4-5$ Gyr.
...
2. **spirals** (S): disks with spiral arms, mix of old and young stars, contain gas and dust. classified by:
   - presence of bar: SA (no bar), SAB (intermediate), SB (barred)
   - tightness of arms and bulge size: a → b → c → d (less tightly wound, smaller bulge)

3. **irregulars** (Irr): no clear morphological pattern, often gas-rich, often interacting

plus **lenticulars** (S0): a transition between E and S — disk but no spiral arms, gas-poor. probably formed from spirals that lost their gas.

---

## the morphology-density relation

(Dressler 1980 and others.)

galaxy morphology depends on **environment**:
- in dense regions (cluster cores): mostly E and S0
- in low-density regions (field): mostly spirals and irregulars
...
galaxies fall into two clear families in color-magnitude space:
- **blue cloud**: actively star-forming, often spirals
...
several remarkable correlations between galaxy properties:

| relation | what it says |
|---|---|
| **Tully-Fisher** | $L \propto V_{\rm flat}^4$ for spirals — connects luminosity to rotational velocity |
| **fundamental plane** | $R_e \propto \sigma^{1.4} \langle I\rangle_e^{-0.9}$ for ellipticals |
| **Faber-Jackson** | $L \propto \sigma^4$ for ellipticals — from the FP |
| **mass-metallicity** | more massive galaxies have higher metallicity |
...
galaxies in the local universe are the **end-products** of a long evolutionary history:
- formed at $z \sim 5-10$ as small dark matter halos collapsed and accreted gas
- merged hierarchically into larger halos
- went through phases of intense star formation (cosmic noon, $z \sim 2$)
